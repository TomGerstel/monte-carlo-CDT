...
Note that by performing the moves in this way in this data structure, the computational effort of performing a move does not scale with the size of the lattice and the computational complexity is $\order{1}$
...
Note that taking this measurement has a computational complexity of $\order{N}$ as it needs to loop through all triangles.
As measurements need to happen far less frequently than performing Markov chain Monte Carlo moves it makes sense to choose a data structure for which the moves have lower computational complexity.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{img/triangulation.pdf}
